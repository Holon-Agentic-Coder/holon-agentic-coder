...
def parse_actual_metrics(
    ...
    # 3. Compute patch size via git diff against plan_branch
    try:
        base_ref = plan_branch if plan_branch else "HEAD~1"
        res = subprocess.run(
            ["git", "diff", "--shortstat", base_ref],
            cwd=repo_dir,
            capture_output=True,
            text=True,
            check=False,
    ...
    # Heuristic cost based on actual vs predicted, slightly lower when cleanly executed
    actual.cost = round(max(0.5, predicted.cost * (0.85 if actual.p_success == 1.0 else 1.1)), 2)

    # Actual entropy based on state surface area and modifications
    base_entropy = 0.4
    if actual.files_changed > 0:
        base_entropy += min(2.0, actual.files_changed * 0.15 + (actual.insertions + actual.deletions) * 0.002)
    actual.entropy = round(min(predicted.entropy * 1.5, base_entropy), 2)
...
def compute_calibration_deltas(
    ...
    bias_directions = {
        ...
        "cost": "Highly Accurate" if cost_err <= 1.0 else "Conservative Cost Estimate",
        "learning_value": "Perfectly Calibrated",
...
def format_markdown_report(report_data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            f"  $$EV_{{\\text{{pred}}}} = {pred['p_success']:.2f} \\times {pred['impact']:.1f} + 0.5 \\times "
            f"{pred['learning_value']:.1f} - 0.3 \\times {pred['entropy']:.2f} - {pred['cost']:.2f} = "
            f"{pred['ev']:.2f}$$"
        ),
        (
            f"  $$EV_{{\\text{{actual}}}} = {act['p_success']:.2f} \\times {act['impact']:.1f} + 0.5 \\times "
            f"{act['learning_value']:.1f} - 0.3 \\times {act['entropy']:.2f} - {act['cost']:.2f} = "
            f"{act['ev']:.2f}$$"
        ),
...
def generate_calibration(
    ...
    ef = EntropyFactors(
        ssa=(0.6, round(max(0.2, actual.files_changed * 0.1), 1)),
        irr=(0.0, 0.0),
        cl=(0.1, 0.0),
        ser=(0.0, 0.0),
        nov=(0.4, 0.4),
...
def run_calibrate(
    ...
    if not skip_commit:
        try:
            ...
            # Checkout or create calibrated branch off execution_branch
            checkout_cmd = ["git", "checkout", "-B", report.calibrated_branch, execution_branch]
            subprocess.run(checkout_cmd, cwd=repo_dir, capture_output=True, text=True, check=False)
        except Exception as e:
            logger.debug("Git branch checkout error: %s", e)
    ...
    if not skip_commit:
        try:
            subprocess.run(["git", "add", report_rel], cwd=repo_dir, check=False)
