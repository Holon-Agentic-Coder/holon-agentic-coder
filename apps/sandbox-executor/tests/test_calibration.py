...
class TestRunCalibrateWorkflow(unittest.TestCase):
    ...
    def test_run_calibrate_checkout_and_commit(self, mock_makedirs, mock_open, mock_run):
        ...
        checkout_cmd_list = [
            c[0][0] for c in mock_run.call_args_list if c[0] and c[0][0][0:3] == ["git", "checkout", "-B"]
        ]
    ...
    @patch("builtins.open", new_callable=unittest.mock.mock_open)
    @patch("os.makedirs")
    def test_run_calibrate_checkout_failure(self, mock_makedirs, mock_open, mock_run):
        mock_res = MagicMock()
        mock_res.returncode = 128
        mock_res.stderr = "fatal: not a valid object name"
        mock_run.return_value = mock_res
        branch = "I-123-intent/P-456-plan/E-789-exec/_"

        with self.assertRaises(RuntimeError) as cm:
            run_calibrate(branch, repo_dir="/tmp/test_repo", skip_commit=False)
        self.assertIn("Failed to checkout calibrated branch", str(cm.exception))
